...
async def cashfree_webhook(request: Request):
    ...
    transaction_response = (
        supabase.table("payment_transactions")
        .select("order_id, employer_id, status")
        .eq("order_id", order_id)
        .maybe_single()
        .execute()
    )
    ...
    if event_type == "PAYMENT_SUCCESS_WEBHOOK":
        supabase.rpc("process_subscription_payment_success", {
            "p_order_id": order_id,
        ...
        mapped_status = _payment_status(event_type.replace("PAYMENT_", "").replace("_WEBHOOK", ""))
