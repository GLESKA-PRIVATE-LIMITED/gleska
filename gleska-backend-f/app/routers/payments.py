"""Employer subscription and worker payment endpoints."""
...
from app.services.cashfree_payment_service import (
...
def _validate_paid_order(
    cashfree_order: dict[str, Any],
    order_id: str,
    transaction: dict[str, Any],
) -> None:
    if (
        cashfree_order.get("order_id") != order_id
        or not cashfree_order.get("cf_order_id")
    ):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="PAYMENT_PROVIDER_ORDER_INVALID",
        )

    if (
        _payment_status(
            str(cashfree_order.get("order_status"))
        )
        != "SUCCESS"
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="PAYMENT_NOT_SUCCESSFUL",
        )

    try:
        provider_amount = Decimal(
            str(cashfree_order.get("order_amount"))
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="PAYMENT_PROVIDER_AMOUNT_INVALID",
        ) from None

    provider_currency = str(
        cashfree_order.get("order_currency", "")
    ).upper()
    ...
    if (
        provider_amount != local_amount
        or provider_currency != local_currency
    ):
        raise HTTPException(
            status_code=status.HTTP_502_BAD_GATEWAY,
            detail="PAYMENT_PROVIDER_DETAILS_MISMATCH",
        )


def _validate_success_webhook(
    order: dict[str, Any],
    ...
    if (
        ...
        or not order.get("cf_order_id")
        or payment.get("payment_status") != "SUCCESS"
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="PAYMENT_WEBHOOK_DETAILS_INVALID",
        )
...
@router.post(
    "/employer/create-commission-order",
...
async def create_employer_commission_order(
    ...
    # ---------------------------------------------------------
    # 1. Authoritative check: job exists and belongs to employer
    # ---------------------------------------------------------
    job_resp = (
        supabase.table("jobs")
    ...
    if not (worker_resp.data or {}).get("id"):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="WORKER_NOT_FOUND",
        )

    # ---------------------------------------------------------
    # 3. Authoritative check: match exists
    # ---------------------------------------------------------
    match_resp = (
    ...
    if match_data.get("status") == "ACCEPTED":
        raise HTTPException(
            ...
            detail="WORKER_ALREADY_DISPATCHED",
        )

    # ---------------------------------------------------------
    # 4. Prevent duplicate successful commission
    #
    # IMPORTANT:
    # INDIVIDUAL_COMMISSION transactions are employer-owned.
    # Therefore worker_profile_id is NULL in payment_transactions.
    #
    # Worker identity is stored in raw_webhook_payload.
    # ---------------------------------------------------------
    existing_success = (
    ...
    for row in (existing_success.data or []):
        payload = row.get("raw_webhook_payload") or {}
        ...
        if (
            ...
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="COMMISSION_ALREADY_PAID",
            )
    ...
    #    this exact employer + job + worker.
    #
    #    Worker ID is stored in raw_webhook_payload because
    #    worker_profile_id must remain NULL for employer-owned
    #    transactions.
    # ---------------------------------------------------------
    pending = (
        supabase.table("payment_transactions")
    ...
    for stale in (pending.data or []):
        payload = stale.get("raw_webhook_payload") or {}

        pending_worker_id = payload.get(
            "worker_profile_id"
        )
    ...
    try:
        ...
        logger.exception(
            "Cashfree commission order creation failed: "
            "employer_id=%s job_id=%s worker_profile_id=%s",
            employer["id"],
            request.job_id,
    ...
    # ---------------------------------------------------------
    # 8. Persist local payment transaction
    #
    # IMPORTANT:
    # employer_id = employer owner
    # worker_profile_id = NULL
    # job_id = direct association
    # worker_profile_id = metadata association
    #
    # This satisfies payment_transactions_owner_check.
    # ---------------------------------------------------------
    ...
    try:
        ...
        logger.exception(
            "Failed to persist commission payment transaction: "
            "employer_id=%s job_id=%s worker_profile_id=%s "
            "order_id=%s",
            employer["id"],
            request.job_id,
        ...
        raise HTTPException(
            ...
            detail="PAYMENT_TRANSACTION_CREATE_FAILED",
        ) from None
...
async def verify_payment(
    ...
    if not transaction:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="PAYMENT_ORDER_NOT_FOUND",
        )
    ...
    try:
        cashfree_order = (
            await CashfreePaymentService.get_order_status(
                order_id
            )
        )
    except CashfreePaymentError as exc:
    ...
    payment_status = _payment_status(
        str(cashfree_order["order_status"])
    )
    ...
    if payment_status == "SUCCESS":
        ...
        rpc_value = (
            ...
            if isinstance(rpc_result, str)
                ...
                next(iter(rpc_result.values()), None)
        ...
        if rpc_value not in {
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="PAYMENT_SUCCESS_PROCESSING_FAILED",
            )
    ...
    return PaymentStatusResponse(
        ...
        status=payment_status,
        subscription_valid_until=(
            employer.get("subscription_valid_until")
        ),
    )
...
async def get_payment_history(
    ...
    if user.role not in {"WORKER", "EMPLOYER"}:
        raise HTTPException(
            ...
            detail="Payment history is unavailable for this role",
        )
    ...
    if user.role == "WORKER":
        ...
        query = (
            supabase.table("payment_transactions")
            .select(
                "id, order_id, payment_category, amount, "
                "currency, status, created_at, updated_at, "
                "payment_success_at, subscription_valid_from, "
                "subscription_valid_until, worker_profile_id"
            )
            .eq(
                "worker_profile_id",
                owner["id"],
        ...
        query = (
            ...
            .select(
                ...
                "currency, status, created_at, updated_at, "
                "payment_success_at, subscription_valid_from, "
                "subscription_valid_until, job_id, "
                "worker_profile_id, raw_webhook_payload"
            )
            .eq(
                "employer_id",
                owner["id"],
    ...
    worker_ids = [
        str(row["worker_profile_id"])
        for row in rows
        if row.get("worker_profile_id")
    ]

    # ---------------------------------------------------------
    # Individual commission worker IDs are stored in
    # raw_webhook_payload because payment_transactions uses
    # employer_id as the owner for those payments.
    # ---------------------------------------------------------
    for row in rows:
        if (
            row.get("payment_category")
            == "INDIVIDUAL_COMMISSION"
            and not row.get("worker_profile_id")
        ):
            payload = row.get(
                "raw_webhook_payload"
            ) or {}
    ...
    for row in rows:
        ...
        # Individual commission worker association
        # comes from raw metadata.
        if (
            category == "INDIVIDUAL_COMMISSION"
            and not worker_profile_id
        ):
            payload = row.get(
                "raw_webhook_payload"
            ) or {}

            if payload.get("worker_profile_id"):
                worker_profile_id = str(
                    payload["worker_profile_id"]
                )
        ...
        history.append(
            PaymentHistoryItem(
                ...
                payment_success_at=row.get(
                    "payment_success_at"
                ),
                subscription_valid_from=row.get(
                    "subscription_valid_from"
                ),
                subscription_valid_until=row.get(
                    "subscription_valid_until"
                ),
                validity_status=_history_validity_status(
                    category,
                    row.get(
                        "subscription_valid_until"
                    ),
...
@router.post(
    "/worker/create-subscription-order",
    response_model=SubscriptionOrderResponse,
)
async def create_worker_subscription_order(
    user: UserResponse = Depends(require_worker),
):
    worker = await _worker(user)

    # ---------------------------------------------------------
    # Expire stale worker subscription payments
    # ---------------------------------------------------------
    pending = (
        supabase.table("payment_transactions")
        .select("id")
    ...
    try:
        order = (
            await CashfreePaymentService.create_subscription_order(
                ...
                amount=200.0,
                order_note="Worker Monthly Subscription",
                return_path="/worker/subscription",
            )
        )
    except CashfreePaymentError as exc:
        raise HTTPException(
        ...
        .insert(
            {
                ...
                "payment_category": "WORKER_SUBSCRIPTION",
                "amount": 200.0,
                "currency": "INR",
                "status": "PENDING",
                "payment_session_id": order[
                    "payment_session_id"
                ],
                "created_at": timestamp.isoformat(),
                "updated_at": timestamp.isoformat(),
...
@router.post(
    "/worker/verify/{order_id}",
    response_model=PaymentStatusResponse,
)
async def verify_worker_payment(
    order_id: str,
    user: UserResponse = Depends(require_worker),
):
    ...
    return PaymentStatusResponse(
        ...
        status=payment_status,
        subscription_valid_until=(
            worker.get("subscription_valid_until")
        ),
    )
...
async def cashfree_webhook(
    ...
    if (
        not signature
        or not timestamp
        or not CashfreePaymentService.verify_webhook_signature(
            signature,
            timestamp,
            raw_body,
        )
    ...
    if event_type == "PAYMENT_SUCCESS_WEBHOOK":
        _validate_success_webhook(
            order,
            (payload.get("data") or {}).get(
                "payment"
            )
            or {},
            transaction,
        ...
        # -----------------------------------------------------
        # IMPORTANT:
        # Keep the existing processing RPC for now.
        #
        # This needs to be reviewed separately so that
        # INDIVIDUAL_COMMISSION does not accidentally trigger
        ...
        if mapped_status is None:
            logger.info(
                "Ignoring unknown Cashfree payment "
                "webhook event: event_type=%s",
                event_type,
            )
            return {"status": "IGNORED"}
    ...
    logger.info(
        "Cashfree payment webhook processed: "
        "order_id=%s event_type=%s",
        order_id,
        event_type,
    )
