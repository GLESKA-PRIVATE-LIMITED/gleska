...
def attendance_http_error(exc: AttendanceError) -> HTTPException:
    mapping = {
    ...
    code, detail = mapping.get(exc.code, (500, "ATTENDANCE_OPERATION_FAILED"))
...
async def worker_check_in(request: WorkerCheckInRequest, user: UserResponse = Depends(require_worker)):
    try:
        ...
        raise attendance_http_error(exc) from exc
