  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#040d1e] px-6 text-white">
      <div className="w-full max-w-md rounded-3xl border border-slate-700 bg-slate-900 p-8 shadow-2xl">
        <Link href="/" className="mb-8 inline-flex items-center gap-2 text-sm font-semibold text-blue-300"><ArrowLeft size={16} /> {t('shared.back')}</Link>
        {step === "success" ? <><h1 className="font-(--font-anton) text-4xl uppercase">{t('auth.passwordUpdated')}</h1><p className="mt-4 text-sm text-slate-400">{t('auth.passwordResetSuccess')}</p><Link href="/" className="mt-8 block w-full rounded-xl bg-blue-600 py-3 text-center font-bold">{t('shared.continueLogin')}</Link></> : <>
          <h1 className="font-(--font-anton) text-4xl uppercase">{t('shared.resetPassword')}</h1>
          <p className="mt-3 text-sm text-slate-400">{step === "phone" ? t('shared.enterPhone') : step === "otp" ? t('shared.enterCode', { phone: phone || t('shared.phone') }) : t('auth.newPasswordHelper')}</p>
          {step === "phone" && <form onSubmit={requestOTP} className="mt-8 space-y-4"><div className="flex items-center rounded-xl border border-slate-600 bg-slate-800 px-4"><span className="mr-2 text-sm font-semibold text-slate-300">+91</span><input required inputMode="numeric" value={phone.replace(/^91/, "")} onChange={(event) => setPhone(event.target.value.replace(/\D/g, "").slice(0, 10))} placeholder={t('auth.mobileLabel')} className="min-w-0 flex-1 bg-transparent py-3 text-white outline-none" /></div><button disabled={submitting} className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 font-bold disabled:opacity-50">{submitting && <Loader2 size={16} className="animate-spin" />}{t('shared.sendOtp')}</button></form>}
          {step === "otp" && <form onSubmit={verify} className="mt-8 space-y-4"><input autoFocus required inputMode="numeric" autoComplete="one-time-code" maxLength={6} value={otp} onChange={(event) => setOtp(event.target.value.replace(/\D/g, "").slice(0, 6))} placeholder="000000" className="w-full rounded-xl border border-slate-600 bg-slate-800 px-4 py-4 text-center text-2xl font-bold tracking-[0.4em] text-white outline-none" /><button disabled={submitting || otp.length !== 6} className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 font-bold disabled:opacity-50">{submitting && <Loader2 size={16} className="animate-spin" />}{t('shared.verifyOtp')}</button><div className="flex justify-between text-xs"><button type="button" onClick={() => { setStep("phone"); setOtp(""); }} className="text-slate-400">{t('auth.changePhone')}</button><button type="button" disabled={Boolean(countdown) || submitting} onClick={resend} className="text-blue-300 disabled:opacity-50">{countdown ? t('shared.resendIn', { seconds: countdown }) : t('shared.resendOtp')}</button></div></form>}
          {step === "password" && <form onSubmit={reset} className="mt-8 space-y-4"><input required type="password" minLength={8} value={password} onChange={(event) => setPassword(event.target.value)} placeholder={t('shared.newPasswordLabel')} className="w-full rounded-xl bg-slate-800 px-4 py-3 outline-none" /><input required type="password" minLength={8} value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder={t('shared.confirmPassword')} className="w-full rounded-xl bg-slate-800 px-4 py-3 outline-none" /><button disabled={submitting || password.length < 8 || password !== confirmPassword} className="flex w-full items-center justify-center gap-2 rounded-xl bg-blue-600 py-3 font-bold disabled:opacity-50">{submitting && <Loader2 size={16} className="animate-spin" />}{t('shared.resetPassword')}</button></form>}
        </>}
      </div>
    </main>
  );
