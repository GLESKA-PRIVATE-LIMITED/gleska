  ...
  return (
    <AccountManagementShell kind="worker" name={user.name} accountLabel={profile.account_type || "EMPLOYEE"} profileHref="/worker/profile" onLogout={() => void logout()}>
      <WorkerPageFrame className="text-slate-900 dark:text-slate-100">
          ...
          </section><aside className="space-y-6"><WorkspaceCard><h2 className="text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Payment</h2><p className="mt-3 text-sm text-slate-600 dark:text-slate-300">Payments are securely handled through Cashfree checkout.</p></WorkspaceCard><WorkspaceCard><h2 className="text-sm font-bold uppercase tracking-wide text-slate-500 dark:text-slate-400">Payment History</h2><p className="mt-3 text-sm text-slate-600 dark:text-slate-300">Only the current subscription status is available here.</p></WorkspaceCard></aside></div>
      </WorkerPageFrame>
