...
-- Add work duration in days and daily work timing columns to public.jobs
ALTER TABLE public.jobs
  ADD COLUMN IF NOT EXISTS work_duration_days INTEGER CHECK (work_duration_days > 0),
  ADD COLUMN IF NOT EXISTS work_timing TEXT;

-- Update create_job_for_employer to accept work_duration_days and work_timing
...
BEGIN
  ...
  VALUES (
    ...
    p_job_site_id,
    p_title,
    p_headcount_required,
    p_max_daily_salary,
    p_min_experience,
    p_trade_id,
    COALESCE(p_required_skills, '{}'::TEXT[]),
...
REVOKE ALL ON FUNCTION public.create_job_for_employer(UUID, UUID, TEXT, INTEGER, NUMERIC, INTEGER, TEXT, TEXT[], INTEGER, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_job_for_employer(UUID, UUID, TEXT, INTEGER, NUMERIC, INTEGER, TEXT, TEXT[], INTEGER, TEXT) TO service_role;
